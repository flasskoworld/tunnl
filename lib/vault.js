...
export function vaultFor(moduleKey, model = null) {
  ...
  return {
    ...
    fields: [
      { label: "The hypothesis this sprint is testing", type: "textarea" },
      { label: `Starting point: ${track.baseline}`, type: "text" },
      { label: "The operating decision", type: "textarea" },
      { label: "The smallest usable artifact", type: "textarea" },
      { label: "The real-world test", type: "textarea" },
      { label: "The pass signal", type: "textarea" },
      { label: "If the test fails", type: "textarea" },
      { label: "The prescribed next move", type: "textarea" },
    ],
  };
}

export function prefillVaultValues(result, setup = {}) {
  const project = setup.focusProject || result?.profile?.building || "this project";
