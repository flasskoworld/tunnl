...
export const VAULT = {
  leverage: {
    ...
    fields: [
      ...
      { label: "Which form of leverage do you have the least of: capital, code, media, or people?", type: "text" },
    ],
  },
  systems: {
    ...
    fields: [
      ...
      { label: "Who or what could run it without you: a person, a tool, or an automation?", type: "text" },
    ],
  },
  strategy: {
  ...
  building: {
    ...
    fields: [
      ...
      { label: "Set a public deadline and name where you'll post it", type: "text" },
  ...
  focus: {
    ...
    fields: [
      ...
      { label: "Kill or freeze everything below #2. Name the date you'll do it.", type: "textarea" },
      { label: "When will you check each week that the tunnel is still pointed right?", type: "text" },
    ],
  },
  forces: {
...
const completedMove = (text = "") => {
  ...
  const rewritten = rewrites.reduce(
    (value, [pattern, replacement]) => pattern.test(value) ? value.replace(pattern, replacement) : value,
    text
  );
  return rewritten.replace(/[.!?]+$/, "") + ".";
};

const EXAMPLE_BASELINES = {
  leverage: "The repeated work took 6 active hours across the last week.",
  systems: "Two of the last five runs followed a written process without clarification.",
  strategy: "Two of the last five responses repeated the same audience and desired outcome.",
  building: "Two finished assets reached another person in the last 30 days.",
  ownership: "I can directly reach two of every five current audience, customer, or member relationships.",
  network: "Two qualified shares, referrals, introductions, or member exchanges occurred last month.",
  economy: "The current offer earned $800 from 16 active delivery hours in the last 30 days.",
  focus: "Five projects and three channels received active work last week.",
  forces: "Two of the last five decisions came from direct customer, audience, or member evidence.",
};
...
export function vaultFor(moduleKey, model = null, context = {}) {
  ...
  return {
    ...
    completedExample: [
      { label: "Starting point", value: EXAMPLE_BASELINES[moduleKey] },
      { label: "Decision", value: completedMove(track.moves.find((move) => move.key === "decision")?.detail) },
      { label: "Work", value: completedMove(track.moves.find((move) => move.key === "artifact")?.detail) },
      { label: "Result", value: `The test passed when ${track.passSignal.charAt(0).toLowerCase()}${track.passSignal.slice(1)}` },
    ],
    fields: [
      { key: "baseline", label: track.baseline, prompt: "Record what is true today. Use a number or observable fact when possible.", timing: "Use now", required: true, type: "textarea" },
      { key: "decision", label: project ? "What decision does this work require?" : "What decision are you making?", prompt: project ? projectContext.moves.decision : "Write the choice and the tradeoff in one clear sentence.", timing: "Use now", required: true, type: "textarea" },
      { key: "work", label: project ? "What will you make or test next?" : "What will you make or test?", prompt: project ? projectContext.moves.artifact : "Name the smallest artifact or real-world test you will complete.", timing: "Use now", required: true, type: "textarea" },
      { key: "result", label: "What happened?", prompt: "Return after the test and record the signal, result, or new constraint.", timing: "After testing", required: false, type: "textarea" },
    ],
  };
}
