...
export async function saveSprintOutcome(userId, outcome) {
  const res = await db().query(
    `INSERT INTO sprint_outcomes
       (user_id, sprint_id, business_model, project_intent, focus_project,
        outcome_text, result_status, attribution, permission_to_publish)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, true)
     ON CONFLICT (user_id, sprint_id) DO UPDATE SET
       business_model = EXCLUDED.business_model,
       project_intent = EXCLUDED.project_intent,
       focus_project = EXCLUDED.focus_project,
       ...
       approved_at = NULL,
       updated_at = now()
     RETURNING id, approved_at, created_at`,
    [
      userId,
      outcome.sprintId,
      outcome.businessModel || null,
      outcome.projectIntent || null,
      outcome.focusProject || null,
      outcome.outcomeText,
...
export async function getPublicSprintOutcomes(limit = 3) {
  const safeLimit = Math.max(1, Math.min(Number(limit) || 3, 6));
  const res = await db().query(
    `SELECT id, project_intent, outcome_text, result_status, attribution
