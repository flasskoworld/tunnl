...
export function recommendedSprintContext(brief = "", { model = "creator", primaryModule = "focus" } = {}) {
  ...
  const hypothesis = interventionFor(model, primaryModule)?.hypothesis?.trim().replace(/[.!?]+$/, "");
  const focus = hypothesis
    ? `Test whether ${hypothesis.charAt(0).toLowerCase()}${hypothesis.slice(1)}.`
    : MODULE_FOCUSES[primaryModule] || MODULE_FOCUSES.focus;
  return {
    ...context,
