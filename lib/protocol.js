...
export function buildProtocol(memo, profile, checkpoint = {}) {
  ...
  const sprintFocus = profile?.focusProject?.trim() || "";
  const projectContext = projectBriefContext(profile?.projectBrief || sprintFocus);
  ...
  const moveDay = (day, priority, move, correctionLead = "") => {
    ...
    return {
      ...
      title: move.title || `${moduleLabel(priority.module)} move`,
      ...
      sprintFocus,
      projectIntent: projectContext.label,
      commission: projectContext.moves[move.key] || projectContext.moves.artifact,
    };
  };
  ...
  const startingTrack = trackFor(p1);
  const startingMetric = profile?.targetMetric?.trim();
  const providedBaseline = profile?.baselineValue?.trim();
  days.push({
    day: 1,
    type: "kickoff",
    module: p1.module,
    title: "Mark your starting point",
    detail: `Your reading points to ${moduleLabel(p1.module)} as the best place to begin. Record one honest number, recent result, or observable fact before you make changes.`,
    minutes: 5,
    why: "A clear starting point gives the sprint a real before-and-after comparison.",
    doneWhen: "One honest starting point is saved and can be compared with your result on Day 14.",
    reflection: "What makes this the most useful evidence to watch during the sprint?",
    methodStage: "diagnose",
    interventionId: interventionKey(startingTrack?.id || `${model}.${p1.module}`, "baseline"),
    methodVersion: METHOD_VERSION,
    startingPointPrompt: startingTrack?.baseline || `What is true in ${moduleLabel(p1.module)} today?`,
    startingPointSuggestion: startingMetric && providedBaseline ? `${startingMetric}: ${providedBaseline}` : "",
    sprintFocus,
  });
