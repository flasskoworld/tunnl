    ...
    p.actions.forEach((action, aIdx) => {
      ...
      if (days.length === 6) {
        days.push({
          ...
          doneWhen: "You have named the clearest signal, the main friction, and how the remaining week should change.",
          reflection: "What is the work telling you that the starting diagnosis could not?",
          methodStage: "adjust",
          interventionId: interventionKey(7),
        });
      }
      const day = days.length + 1;
      const correction = day > 7 ? courseCorrectionMode(checkpoint) : "continue";
      const correctionLead = correction === "narrow"
        ? "Reduce this to the smallest version that can create a real signal. "
        : correction === "change"
          ? `Use this move against the revised constraint${checkpoint.revisedConstraint ? `: ${checkpoint.revisedConstraint}` : " you named on Day 7"}. `
          : "";
      days.push({
        day,
        type: "action",
        ...
        interventionId: interventionKey(day),
