...
export function buildProtocol(memo, profile, checkpoint = {}) {
  ...
  const target = profile?.targetMetric
    ? `${profile.targetMetric}: ${profile.baselineValue || "not recorded"} -> ${profile.targetValue || "movement by Day 14"}`
    : profile?.successMeasure || "the outcome named during setup";
  ...
  days.push({
    ...
    detail: `"${memo.verdict}" Your Sprint Target is ${target}. Before moving, confirm that the constraint and starting measure reflect the work as it is today.`,
    minutes: 15,
    why: "A plan only works after you decide which diagnosis you are willing to act on.",
    doneWhen: profile?.targetMetric
      ? `You have confirmed ${profile.targetMetric} starts at ${profile.baselineValue || "the value you recorded"} and movement means ${profile.targetValue || "the Day 14 target"}.`
      : "You have written one sentence you accept, one sentence you resist, and why.",
    reflection: "What makes this the right constraint and measure for the next 14 days?",
    methodStage: "diagnose",
  });

  trackFor(p1).moves.forEach((move, index) => days.push(moveDay(index + 2, p1, move)));
  days.push(integrationDay(5, p1, "Before starting the next priority, compare the three moves with the baseline and Sprint Target."));
  ...
  days.push({
    ...
    detail:
      `Your Sprint Target was ${target}. Record the finishing value, decide whether the target moved, and choose the next action from the evidence.`,
    minutes: 25,
    why: "The sprint is complete only when the work changes what you will do next.",
    doneWhen: profile?.targetMetric || profile?.successMeasure
      ? `You have compared the finishing result against ${target}, then recorded the strongest result, unresolved constraint, and next commitment.`
