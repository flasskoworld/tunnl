...
export const UNIVERSAL_QUESTIONS = [
  ...
  { id: "capacity", field: "weeklyCapacity", eyebrow: "Capacity", q: "How much protected time did this work receive last week?", options: [
    ...
    { t: "No protected time", v: "0 hours", w: { focus: 1, systems: 1 } },
  ]},
  ...
  { id: "primary_constraint", field: "blocker", eyebrow: "Constraint", q: "Which constraint is costing you the most momentum right now?", options: [
    { t: "Not enough protected time", v: "Time", w: { focus: 2, systems: 3 } },
...
function contextualAction(action, profile, index) {
  const model = MODEL_CONTEXT[profile.business_model] || MODEL_CONTEXT.creator;
  const target = profile.focusProject || model.label;
  const audience = profile.audience ? ` for ${profile.audience}` : "";
  const leads = [
    `${model.actionLead}, apply this to ${target}${audience}:`,
    `Use the evidence you already have from ${profile.recentEvidence || "the last 30 days"}:`,
    `Before adding another idea or channel:`,
  ];
  return `${leads[index] || leads[0]} ${action}`;
}
...
export function buildFallbackMemo(archetypeKey, weak, profile = {}) {
  ...
  return {
    ...
    priorities: weak.map((k) => ({
      ...
      actions: FALLBACK_DIAGNOSES[k].actions.map((action, index) => contextualAction(action, profile, index)),
    })),
    risks: [
