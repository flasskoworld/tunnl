...
export const WORK_QUESTION = {
  ...
  placeholder: "I am launching a paid workshop for independent designers and want to validate the offer, enroll the first five people, and create a repeatable launch process.",
  context: true,
  textInput: true,
};
...
function workLine(profile) {
  if (!profile.focusProject) return null;
  const work = profile.focusProject.trim().replace(/[.!?]+$/, "");
  return `The work in view is: ${work}. The recommendations below are built to move this specific work forward.`;
}
...
export function buildFallbackMemo(archetypeKey, weak, profile = {}) {
  ...
  return {
    ...
    priorities: weak.map((k) => {
      ...
      return {
        module: k,
        diagnosis: `${FALLBACK_DIAGNOSES[k].diagnosis} In your ${MODEL_CONTEXT[profile.business_model]?.label || "business"}, this is showing up against ${profile.recentEvidence || "the evidence you reported"}.${profile.focusProject ? ` For the work you named — ${profile.focusProject} — this is the constraint the intervention will test.` : ""}`,
