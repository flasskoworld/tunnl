...
export const WORK_QUESTION = {
  ...
  prompt: "Describe the specific project, offer, launch, or outcome you want to move forward. Include who it is for and what you are trying to make happen.",
  placeholder: "Example: validate a new offer with five target customers and decide whether it is ready to launch.",
  context: true,
  textInput: true,
};
...
export function buildDiagnosticQuestions(model, answers = []) {
  ...
  return [...base, ...weakestModules(computeScores(answers), 2).map((key) => FOLLOW_UPS[key]), WORK_QUESTION];
}
...
export const ARCHETYPES = {
  ...
  STUCK_OPTIMIZER: {
    ...
    line: "Immaculate systems and sharpened focus, pointed at low-leverage work. Motion without acquisition.",
  },
};
...
export const FALLBACK_DIAGNOSES = {
  ...
  forces: {
    ...
    actions: [
      "Audit your top five information inputs and check who profits from each",
...
function workLine(profile) {
  const brief = profile.projectBrief || profile.focusProject;
  if (!brief) return null;
  const work = brief.trim().replace(/[.!?]+$/, "");
  const focus = profile.projectBrief && profile.focusProject
    ? ` Tunnl has shaped that into a 14-day focus: ${profile.focusProject}`
    : "";
  return `The work in view is: ${work}.${focus}`;
}
...
export function buildFallbackMemo(archetypeKey, weak, profile = {}) {
  ...
  return {
    ...
    priorities: weak.map((k) => {
      ...
      return {
        module: k,
        diagnosis: `${FALLBACK_DIAGNOSES[k].diagnosis} In your ${MODEL_CONTEXT[profile.business_model]?.label || "business"}, this is showing up against ${profile.recentEvidence || "the evidence you reported"}.${profile.projectBrief || profile.focusProject ? ` For the work you named, ${profile.projectBrief || profile.focusProject}, this is the constraint the intervention will test.` : ""}`,
